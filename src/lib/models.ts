...
export const isValidEventType = (type: string): type is EventType => {
  return ['state_change', 'progress_update', 'rating_added', 'note_added'].includes(type);
};
...
export const validateProgress = (progress: Book['progress']): boolean => {
  ...
  if (progress.percentage && (progress.percentage < 0 || progress.percentage > 100)) return false;
  return true;
};
...
export const canTransitionTo = (currentState: ReadingState, newState: ReadingState): boolean => {
  return READING_STATE_TRANSITIONS[currentState].includes(newState as ReadingState);
};