 ...
 */
export const mapCameraError = (error: unknown): ScanningError => {
  ...
  if (
    ...
    return {
      message:
        "Camera configuration issue. Your camera doesn't support the required settings.",
      suggestions: [
 ...
 */
export const handleBarcodeDetectionError = (
  context: "camera" | "upload"
): ScanningError => {
  if (context === "camera") {
    return {
 ...
 */
export const getBarcodeQualityTips = (
  issue: "blurry" | "lighting" | "angle" | "distance"
): string[] => {
  switch (issue) {
    case "blurry":
      return [
 ...
 */
export const handleBookLookupError = (error: unknown): ScanningError => {
  const err = error as { message?: string } | null;
  const errorMessage =
    err?.message?.toLowerCase() || String(error).toLowerCase();
